...
def draw_frame_to_image(image, blocks, colon_pixels=None, scale=1):
    """
    Draw blocks and colon onto a PIL Image.

    Args:
        image: PIL Image (64x32 RGB)
        blocks: List of (blocktype, color_index, abs_x, abs_y, rotation) tuples
...
class PNGRenderer:
    ...
    def __init__(self, output_dir, png_scale=8, pixel_scale=1):
    ...
    def render_frame(self, blocks, colon_pixels=None):
        ...
        draw_frame_to_image(image, blocks, colon_pixels, self.pixel_scale)

        # Scale up for visibility
        scaled = image.resize(
            (64 * self.png_scale, 32 * self.png_scale),
            Image.NEAREST,
        )
...
class MatrixRenderer:
    ...
    def __init__(self, matrix, pixel_scale=1):
        self.matrix = matrix
        self.pixel_scale = pixel_scale
